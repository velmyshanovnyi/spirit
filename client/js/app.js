...
import { getGroup, updateGroupMembers, ensureGroupBootstrap } from "./groups.js";
import { initGroupsUI } from "./groupsUI.js";
import { initDeviceLinkingUI } from "./deviceLinkingUI.js";
import { initRecoveryUI } from "./recoveryUI.js";
import { initGroupMesh } from "./groupMesh.js";
import { initServerConfigUI } from "./serverConfigUI.js";
import { initImportedContactsUI } from "./importedContactsUI.js";
import { initContactsUI } from "./contactsUI.js";
...
export function initApp(doc, options) {
  ...
  const { renderRecoveryCard, drainRecoveryShareOutboxForPeer } = initRecoveryUI({
    doc,
    el,
    t,
    state,
    ...
    readSessionTtlHours,
    postIdentityRoute,
    // Lazy thunk: `router` is a const created later in initApp; handlers
    // only navigate at click-time, long after it exists.
  ...
  const { renderImportedContactsScreen } = initImportedContactsUI({ doc, el, t, state });

  const { renderContactsScreen, checkContactProofs } = initContactsUI({
    doc,
    el,
    t,
    state,
    // doc.defaultView directly: app.js's own `const win` is created later
    // in initApp (next to onScreenChange) -- naming it here would hit its TDZ.
    win: doc.defaultView,
    navigate: (route) => router.navigate(route),
    applyContactsFilter,
