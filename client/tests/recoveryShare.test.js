...
import { splitSecret, combineShares } from "../js/shamir.js";

const SHARE = { x: 3, y: new Uint8Array([1, 2, 3, 4, 5]), threshold: 2, totalShares: 3 };

describe("Section A2 (specs/reviews/spirit-evaluation-triage.md): real splitSecret() output round-trips end to end", () => {
  // The hand-built SHARE mock above (threshold/totalShares added manually)
  // masked a real integration bug: splitSecret's actual return value never
  // carried those fields, so every share generated by the real "Налаштувати
  // відновлення" UI flow text-encoded as "spirit-share:1.undefined.undefined...."
  // and decodeShareFromText rejected it. This test exercises the REAL
  // producer (splitSecret), not a mock, end to end through both wire
  // encodings.
  it("a share produced by splitSecret() text-encodes and decodes back to the same share", () => {
    const secret = crypto.getRandomValues(new Uint8Array(32));
    const [share] = splitSecret(secret, { threshold: 2, shares: 3 });
    ...
    expect([...decoded.y]).toEqual([...share.y]);
  });

  it("a share produced by splitSecret() round-trips through buildRecoveryShareAnnounce/parseRecoveryShareAnnounce", () => {
    const secret = crypto.getRandomValues(new Uint8Array(32));
    const shares = splitSecret(secret, { threshold: 3, shares: 4 });

    const announce = buildRecoveryShareAnnounce(shares[0]);
    const parsed = parseRecoveryShareAnnounce(announce);
    expect(parsed).not.toBeNull();

    const recovered = combineShares([parsed, ...shares.slice(1, 3)]);
