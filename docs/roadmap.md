...
**Пріоритет для наступного волонтера (зафіксовано 2026-07-18)**: справжній відкритий гап, виявлений під час перевірки пункту вище — **TOFU при ПЕРШІЙ зустрічі з контактом**. Автентифікований обмін ключами (вище) захищає ECDH-сесію ПІСЛЯ того, як identity-ключ контакту вже прийнятий, але сам момент першого прийняття identity-ключа не має позаканального доказу за замовчуванням — лише TOFU. Частково покрито Секціями 2c/2d (proof-публікації, Google OAuth), але це опційні, а не обов'язкові кроки. Перед стартом коду по цьому пункту — окреме дослідження масштабу (чи обмежитись сильнішим UI-nudge до наявних proof-механізмів, чи потрібен новий механізм) і, ймовірно, AskUserQuestion-обговорення з користувачем, оскільки це модель довіри.
