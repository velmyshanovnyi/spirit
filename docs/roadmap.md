...
## Фаза 2 — Профілі, backup та мультипристрій ✅ ЗАВЕРШЕНО (ядро)

Повна специфікація та історія review — [specs/phase2/profiles.md](../specs/phase2/profiles.md). Секції 1-10, test-first (171 тест у сумі по проєкту).

- IndexedDB-обгортка (`db.js`) + passphrase-шифрування сховища (PBKDF2-HMAC-SHA256 600k → AES-256-GCM, `vault.js`).
- Перманентні профілі (`profile.js`): створення/розблокування, non-extractable ключ після завантаження.
- Backup identity-ключа: BIP39-подібна мнемоніка (24 слова, пряме кодування 32-байтного скаляра) та зашифрований keyfile — обидва з відновленням; UI-флоу створення профілю з backup-кроком.
- Мультипристрій (`deviceLinking.js`): device-сертифікати, приєднання нового пристрою через той самий P2P/E2EE-канал (передача identity + контактів), відкликання через версійований підписаний список сертифікатів (анти-replay монотонність).
- Ще не зроблено з первісного плану фази: зашифрована історія повідомлень, мультиакаунти в UI, транспорт розповсюдження device-списку контактам, онбординг-UX ([migration.md](migration.md)) — перенесено в наступні ітерації.
