...
- [x] **Exec review**: 1 ітерація, зійшлося, 6 реальних знахідок
      ...
      `specs/reviews/design-edit-mode-RF21-iter1.md`). **Жива перевірка на
      `spirit.kolo.media` (2026-08-07) підтвердила**: реальне DOM-
      переміщення `#floating-video` з `<body>` у справжню `.card-wide`
      картку розмови й назад; `position: relative` (finding 2) --
      `#video-remote` лишається в межах панелі (`getBoundingClientRect`
      перевірено); inline `width`/`left` коректно очищаються при докінгу
      й відновлюються ТОЧНО (`320px`/`0px` до і після) при undock
      (finding 1). **Що ще НЕ перевірено живо**: неперервність
      відтворення реального `<video>`/`MediaStream` під час дзвінка при
      переміщенні DOM-вузла (Browser pane цього середовища блокує доступ
      до камери/мікрофона) -- відкладено до сесії з доступом до камери
      двох реальних профілів.
