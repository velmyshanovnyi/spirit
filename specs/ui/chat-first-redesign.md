...
## Секція H5: Дефолтна сторінка — чат без реєстрації (НЕ ПОЧАТО, найризикованіша)
...
**Чому не почато в цій сесії**: `ROUTES`/`defaultRoute="account"` і практично весь наявний тест-сьют (166 тестів у `app.test.js`, 449 по проєкту) явно очікують, що свіжий `initApp()` стартує на екрані "account" без жодної фонової мережевої/IndexedDB-активності. Емпірично перевірено (2026-07-17): auto-trigger вимагав би async `listProfiles()` (IndexedDB) виклику при КОЖНОМУ `initApp()`, включно з кожним із 166 наявних тестів — це системна міграція тест-сьюту, не лише додавання нової поведінки, і виходить за межі безпечного обсягу однієї сесії без ретельної секційної розбивки. Наступний волонтер: почніть з малого — окремий helper-функції `shouldAutoStartChat()` (перевірка: немає invite-лінка, немає remembered session, немає stored profiles) з власним тест-файлом, ПОТІМ підключення до `initApp()` в окремому коміті з повним проходом по існуючих тестах.
