...
- [x] **Verify**: живі повторні запити на `spirit.kibr.com.ua` (`server/verify/section9_rate_limiter.php`, 7 перевірок, короткі 2с-вікна) — перші N проходять, N+1 відхилено, незалежні бакети на IP, незалежний (строгіший) ліміт на створення кімнат, вікно скидається з часом, застарілі IP-записи самоочищуються.
- [x] **Impl**: `server/library/RateLimiter.php` — два незалежні sliding-window бакети (`requests`, `room_creations`) в окремому від `database.json` файлі, атомарний запис через temp+`rename()` (як у Storage), навмисно fail-open на пошкоджений файл (на відміну від fail-closed `Storage::load()` — обґрунтовано в коді).
- [x] **Exec review**: 1 ітерація, конвергенція — [iter1](../reviews/mvp-section-9-rate-limiter-iter1.md). Чотири пункти перенесено до Секції 10 (hard cap/LRU, гірший режим відмови гонки під навантаженням, порядок виклику бакетів, ігнороване значення `save()`).

## Секція 10: Сигнальний вузол — actions і `fetch_proof`

- [x] **Verify**: живий повний happy-path на `spirit.kibr.com.ua/spirit/public/index.php` (`create_invite → create_offer → get_offer → submit_answer → check_answer`) із двома різними sender-ідентичностями (симуляція ініціатора/учасника); одноразовість токена (403 після використання); **3/3 раунди конкурентної гонки** — два паралельні `submit_answer` з одним токеном, рівно один `200`, інший `403` кожного разу; коди помилок 400/403/404/405 живо перевірені; CORS preflight `OPTIONS` → `204` до правила 405; rate limiting → `429` після вичерпання бюджету; `fetch_proof` — успішний фетч реального URL, SSRF-блокування приватних IP включно з cloud-metadata (169.254.169.254), decimal/hex/octal-кодуваннями та IPv6 loopback (усі коректно заблоковані, жодного bypass), вимкнено за замовчуванням у фінальному деплої.
  - Усі перенесені пункти з Секцій 6-9 закриті: `LOCK_EX`-лок навколо load→validate→mutate→save (закриває TOCTOU з Секції 7, підтверджено 3/3 живими гонками); контракт Storage "load-or-abort" дотриманий (`RuntimeException` → чиста `500`); hard cap/LRU-витіснення реалізовано і для `database.json` (`Storage::enforceMaxSessions`), і для файлу rate-limit (`RateLimiter`, `maxTrackedIps`); порядок rate-limit бакетів (загальний перед room-creation, short-circuit при відмові) реалізовано.
- [x] **Impl**: `server/public/index.php`, `server/library/SignalingController.php`, `server/config.php`. Супутні зміни до вже конвергованих класів: `InviteManager` рефакторено на чистий маніпулятор `array &$db` (без власного Storage) — необхідно для атомарного лока контролера; `Storage::enforceMaxSessions()` і `RateLimiter`'s `maxTrackedIps` — нові hard-cap методи.
- [x] **Exec review**: 2 ітерації, конвергенція — [iter1](../reviews/mvp-section-10-controller-iter1.md), [iter2](../reviews/mvp-section-10-controller-iter2.md). Знайдено й виправлено 8 проблем (включно з двома, знайденими саме під час живого тестування SSRF-кодувань: некоректна класифікація помилок і невзятий у розрахунок bracket-синтаксис IPv6-літералів).
